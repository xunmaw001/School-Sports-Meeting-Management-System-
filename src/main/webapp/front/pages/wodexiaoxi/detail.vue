...
<script>
	export default {
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','wodexiaoxi');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				uni.setStorageSync('crossTable','wodexiaoxi');
			...
			// 获取详情
			async init(){
				let res = await this.$api.info('wodexiaoxi', this.id);
			...
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../discusswodexiaoxi/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
			...
			async onFinishTap() {
				...
				await this.$api.update('wodexiaoxi', this.detail);
				this.$utils.msg('审核成功');
